...
from resources.lib.youtube_installer import get_latest_youtube_urls, install_youtube_addon
from resources.lib.trakt_installer import install_trakt_addon  # Import per Trakt
...
# Percorsi e settings - Versione migliorata
...
# Controllo aggiornamenti all'avvio - Versione migliorata
...
class RepoManagerGUI(xbmcgui.WindowXML):
        ...
        if controlId == 100:
            ...
            if self.selected_index < len(self.sources):
                ...
                if repo.get('name').lower() == 'youtube':
                    ...
                    choice = xbmcgui.Dialog().select("YouTube Addon", options)
                    if choice < 0:
                        return
                    ...
                    if success:
                        if xbmcgui.Dialog().yesno(
                            ADDON_NAME,
                            "Link aggiunto con successo! Riavviare Kodi ora?",
                            yeslabel="Sì",
                            nolabel="No"
                        ):
                            ...
                            xbmcgui.Dialog().notification(
                                ADDON_NAME,
                                "Riavvio necessario per vedere il file zip in 'Installa da file zip'",
                                ADDON_ICON,
                                3000
                            )
                    return
    ...
    def install_single(self, repo, show_dialog=True):
        ...
        # Gestione speciale per Trakt (non necessario qui, già gestito in onClick)
        # Nota: Trakt è gestito nell'onClick, quindi non arriverà qui.
    ...
    def uninstall_single(self, repo, show_dialog=True):
        ...
        # Esegui la rimozione
        success = uninstall_repo(repo)  # Usa la funzione esistente dal modulo
