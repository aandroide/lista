...
class RepoManagerGUI(xbmcgui.WindowXML):
    ...
    def uninstall_all(self):
        ...
        for i, repo in enumerate(self.sources):
            ...
            try:
                # Repository normali
                if not ("kodinerds" in name or "sandmann" in name):
                    if remove_source_from_xml(repo):
                        removed_count += 1
                # Repository speciali (Kodinerds/Sandmann)
                else:
                    # Kodinerds
    ...
    def uninstall_single(self, repo, show_dialog=True):
        ...
        removed = False
        try:
            # Repository normali (non Kodinerds/Sandmann)
            if not ("kodinerds" in name or "sandmann" in name):
                removed = remove_source_from_xml(repo)
            # Kodinerds e Sandmann: rimozione fisica
            else:
